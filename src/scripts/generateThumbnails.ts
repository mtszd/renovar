...
async function generateThumbnails(folderName: string): Promise<void> {
  ...
  try {
    ...
    for (const file of files) {
      ...
      console.log(
        `Procesando ${file}: ${metadata.width}x${metadata.height} -> ${targetWidth}x${targetHeight}`,
      );
      ...
      console.log(
        `Reducción de tamaño: ${reduction}% (${(originalStats.size / 1024).toFixed(1)}KB -> ${(stats.size / 1024).toFixed(1)}KB)`,
      );
    }
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  generateThumbnails(folderName).catch((error) => {
    console.error(error);
    process.exit(1);
  });
}
